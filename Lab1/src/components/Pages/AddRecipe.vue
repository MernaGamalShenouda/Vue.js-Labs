...
export default {
  ...
  methods: {
    handlePostReq() {
      this.ingredients = this.ingredients.split(',').map(item => item.trim()).filter(Boolean);
    ...
    getPrdById() {
      axios.get(`http://localhost:2000/recipes/${this.id}`)
      .then(p => {
    ...
    handlePutReq() {
      axios.put(`http://localhost:2000/recipes/${this.id}`, {
        name: this.name,
        ingredients: this.ingredients,
        instructions: this.instructions,
        rating: this.rating,
