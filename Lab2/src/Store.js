...
const store = createStore({
    ...
    actions: {
        ...
        createRecipe({ commit }, newRecipe) {
            axios.post("http://localhost:2000/recipes", newRecipe)
            .then(res => commit('ADD_RECIPE', res.data))
            .catch(err => console.error(err));
        },
        updateRecipe({ commit }, updatedRecipe) {
            axios.put(`http://localhost:2000/recipes/${updatedRecipe.id}`, updatedRecipe)
            .then(() => commit('UPDATE_RECIPE', updatedRecipe))
            .catch(err => console.error(err));
        },
        deleteRecipe({ commit }, recipeId) {
            axios.delete(`http://localhost:2000/recipes/${recipeId}`)
